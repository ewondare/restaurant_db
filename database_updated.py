    ...
    with st.container():
        ...
        if submit_button:
            if not item_name or not price or not menu_id:
                ...
                if item_type_selection == 'Appetizer':
                    item_values["Description"] = additional_fields["Description"]
                    item_values["Recipe"] = additional_fields["Recipe"]
                    insert_query = """
                        INSERT INTO Appetizer_item (item_name, price, description, recipe, menu_id)
                        VALUES (:ItemName, :Price, :Description, :Recipe, :MenuId)
                    """
                ...
                if item_type_selection == 'Entree':
                    ...
                    item_values["Recipe"] = additional_fields["Recipe"]
                    insert_query = """
                        INSERT INTO Entree_item (item_name, price, description, recipe, menu_id)
                        VALUES (:ItemName, :Price, :Description, :Recipe, :MenuId)
                    """
                ...
                if item_type_selection == 'Dessert':
                    ...
                    item_values["Recipe"] = additional_fields["Recipe"]
                    insert_query = """
                        INSERT INTO Dessert_item (item_name, price, description, recipe, menu_id)
                        VALUES (:ItemName, :Price, :Description, :Recipe, :MenuId)
                    """

                try:
                    with conn.session as s:
                        s.execute(insert_query, item_values)
                        s.commit()
                    st.success(f"{item_type_selection} item inserted successfully")
